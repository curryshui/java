...
public class BookTypeAddIFrame extends JInternalFrame {
	...
	public BookTypeAddIFrame() {
		...
		label_2.setText("book category name：");
		panel_3.add(label_2);
		...
		label_3.setText("days can borrow：");
		panel_3.add(label_3);
		...
		label_5.setText("fine(1 day)：");
		panel_3.add(label_5);

		fakuan = new JTextField("unit of jiao");
		...
		button.addActionListener(new ActionListener(){
			public void actionPerformed(final ActionEvent e) {
				if(bookTypeName.getText().length()==0){
					JOptionPane.showMessageDialog(null, "please input book category");
					return;
				}
				if(days.getText().length()==0){
					JOptionPane.showMessageDialog(null, "please input days");
					return;
				}
				if(fakuan.getText().length()==0||fakuan.getText().trim().equals("单位为角")){
					JOptionPane.showMessageDialog(null, "please input fine");
					return;
				}
