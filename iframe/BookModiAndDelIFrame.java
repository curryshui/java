...
public class BookModiAndDelIFrame extends JInternalFrame {
	...
	private Object[][] getFileStates(List list){
		String[] columnNames = { "book ISBN", "book category", "book name", "author", "tanslator", "press",
				"publishing date", "price" };
...
//		panel_1.add(button_2);
		...
		Object[][] results=getFileStates(Dao.selectBookInfo());
		columnNames = new String[]{"book ISBN", "book category", "book name", "author", "translator", "press", "publishing date",
				"price"};
		...
		label_1_1.setText("publishing date：");
	...
	class addBookActionListener implements ActionListener {
		public void actionPerformed(final ActionEvent e) {
			...
			if(ISBN.getText().length()!=13){
				JOptionPane.showMessageDialog(null, "please input 13 bits ISBN");
				return;
			}
			...
			if(publisher.getText().length()==0){
				JOptionPane.showMessageDialog(null, "please input press");
				return;
			}
			//日期与单价进行数字验证代码？
			if(pubDate.getText().length()==0){
				JOptionPane.showMessageDialog(null, "please input publishing date");
				return;
			}
