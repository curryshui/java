...
public class newBookOrderIFrame extends JInternalFrame {
	...
	public newBookOrderIFrame() {
		super();
		setTitle("new book order manage");
		setIconifiable(true);
		setClosable(true);
		...
		panel_4.setBorder(new TitledBorder(null, "order information", TitledBorder.DEFAULT_JUSTIFICATION, TitledBorder.DEFAULT_POSITION, null, null));
		...
		panel_1.setBorder(new TitledBorder(null, "book information", TitledBorder.DEFAULT_JUSTIFICATION, TitledBorder.DEFAULT_POSITION, null, null));
		...
		String[] array=new String[]{"***press","**information press","**independent press","***business press"};
	...
	class ButtonAddLisenter implements ActionListener{
		public void actionPerformed(final ActionEvent e) {
			if(orderDate.getText().isEmpty()){
				JOptionPane.showMessageDialog(null, "please input order date");
				return;
			}
			if(ISBN.getText().isEmpty()){
				JOptionPane.showMessageDialog(null, "please input ISBN");
				return;
			}
			...
			if(operator.getText().isEmpty()){
				JOptionPane.showMessageDialog(null, "please input operator");
				return;
			}
	...
	class DateListener extends KeyAdapter {
		public void keyTyped(KeyEvent e) {
			if(orderDate.getText().isEmpty()){
				JOptionPane.showMessageDialog(null, "时间格式请使用\"2015-11-10\"格式");
			}
		}
	}
	...
	class ISBNListenerlostFocus extends FocusAdapter{
		public void focusLost(FocusEvent e){
			String ISBNs = ISBN.getText().trim();
			if(!Dao.selectBookOrder(ISBN.getText().trim()).isEmpty()){
				JOptionPane.showMessageDialog(null, "repeated order information!");
				ISBN.setText("");
				bookName.setText("");
				price.setText("");
				return;
			...
			if(list.isEmpty()&&!ISBN.getText().isEmpty()){
				...
				bookName.setText("");
				price.setText("");
				JOptionPane.showMessageDialog(null, "This ISBN doesn't exist,please add it in data first");
			}
			for (int i = 0; i < list.size(); i++) {
				BookInfo bookinfo = (BookInfo) list.get(i);
				bookName.setText(bookinfo.getBookname());
